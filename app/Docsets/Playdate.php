...
class Playdate extends BaseDocset
{
    ...
    public function grab(): bool
    {
        ...
        $this->neutralizeTocSidebar($destination);
    ...
    /**
     * The Asciidoctor pages reserve a fixed 35rem-wide left gutter for the
     * `#toc` sidebar (removed in removeChrome()). Drop the matching
     * `margin-left` from the bundled stylesheet so content uses the full width
     * in Dash's narrow pane — no empty gutter, no horizontal scrolling.
     */
    protected function neutralizeTocSidebar(string $destination): void
    {
        foreach (File::allFiles($destination) as $file) {
            ...
            if (! Str::contains($css, 'margin-left: 35rem')) {
                continue;
            }

            File::put(
                $file->getPathname(),
                str_replace('margin-left: 35rem', 'margin-left: 0', $css)
            );
        }
    }
     ...
     * neutralizeTocSidebar() removes); Dash provides its own table of contents
